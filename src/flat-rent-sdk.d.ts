...
export const database:IDateBaseJsonItem[];
...
export class FlatRentSdk {
  ...
  //
  get(id: string): Promise<IDateBaseJsonFormated | null> {
   ...
   */
  //
  search(parameters:ISearchParametersObj):Promise<IDateBaseJson | Error> {
  ...
  //
  _areAllDatesAvailable(flat:IDateBaseJsonItem, dateRange:Date[]):boolean {
  ...
  //
  _formatFlatObject(flat: IDateBaseJsonItem, nightNumber?: number): IDateBaseJsonItem {
  ...
  //
  _readDatabase(): null | DateBaseJSON {
  ...
  //
  _writeDatabase(database: DateBaseJSON):void {
    window.localStorage.setItem(localStorageKey, JSON.stringify(database))
  }

  //
  _syncDatabase(database: DateBaseJSON): void {
    this._writeDatabase(database)
    this.database = this._readDatabase()
  }
}
